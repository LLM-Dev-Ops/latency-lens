...
//! 3. Executes the Latency Analysis Agent
//! 4. Returns JSON output conforming to LatencyAnalysisOutput schema
//!
//! # Deployment Model
//!
...
//! # HTTP Endpoints
//!
//! - `POST /analyze` - Perform latency analysis
//! - `POST /inspect` - Inspect a previous analysis
//! - `POST /replay` - Replay analysis with different configuration
...
impl EdgeFunctionHandler {
    ...
    pub async fn handle(&self, request: EdgeFunctionRequest) -> EdgeFunctionResponse {
        info!(
        ...
        match request.operation {
            EdgeOperation::Analyze => self.handle_analyze(request).await,
            EdgeOperation::Inspect => self.handle_inspect(request).await,
            EdgeOperation::Replay => self.handle_replay(request).await,
            EdgeOperation::Health => self.handle_health().await,
        }
    }

    /// Handle analyze operation
    async fn handle_analyze(&self, request: EdgeFunctionRequest) -> EdgeFunctionResponse {
        // Parse input
    ...
    async fn handle_inspect(&self, request: EdgeFunctionRequest) -> EdgeFunctionResponse {
        let inspect_req: InspectRequest = match serde_json::from_value(request.payload) {
            ...
            Err(e) => {
                return EdgeFunctionResponse {
                    ...
                    error: Some(EdgeFunctionError {
                        code: "INVALID_INPUT".to_string(),
                        message: format!("Failed to parse inspect request: {}", e),
                        details: None,
                    }),
                    decision_event_id: None,
                    trace_context: request.trace_context,
        ...
        EdgeFunctionResponse {
            ...
            error: Some(EdgeFunctionError {
                ...
                message: "Must provide event_id, analysis_id, or query".to_string(),
                details: None,
            }),
            decision_event_id: None,
            trace_context: request.trace_context,
    ...
    /// Handle replay operation
    async fn handle_replay(&self, request: EdgeFunctionRequest) -> EdgeFunctionResponse {
        let replay_req: ReplayRequest = match serde_json::from_value(request.payload) {
            ...
            Err(e) => {
                return EdgeFunctionResponse {
                    ...
                    error: Some(EdgeFunctionError {
                        code: "INVALID_INPUT".to_string(),
                        message: format!("Failed to parse replay request: {}", e),
                        details: None,
                    }),
                    decision_event_id: None,
                    trace_context: request.trace_context,
        ...
        // Extract original inputs (would need to be stored in the event)
        // For now, return error indicating replay requires original inputs
        warn!(
        ...
        EdgeFunctionResponse {
            ...
            result: None,
            error: Some(EdgeFunctionError {
                code: "REPLAY_NOT_AVAILABLE".to_string(),
                message: "Replay requires original input metrics to be stored. Original analysis found but inputs are not persisted.".to_string(),
    ...
    /// Handle health check
    async fn handle_health(&self) -> EdgeFunctionResponse {
        match self.ruvector_client.health_check().await {
            Ok(health) => EdgeFunctionResponse {
